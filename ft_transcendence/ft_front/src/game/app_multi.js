...
class Main {
  ...
  static webfunc(get_hash) {
    ...
    const handleKeyUp = (event) => {
      let message = { message: event.key, pid: Main.players, uuid: "" };
      let flag = 0;
      if (event.code === "KeyQ") {
        message = { message: "upstop", pid: Main.players, uuid: "" };
        flag = 1;
      }
      if (event.code == "KeyA") {
        message = { message: "downstop", pid: Main.players, uuid: "" };
        flag = 1;
      }
    ...
    const handleKeyDown = (event) => {
      let message = { message: event.key, pid: Main.players };
      let flag = 0;

      if (event.code === "KeyQ") {
        message = { message: "up", pid: Main.players, uuid: "" };
        flag = 1;
      }
      if (event.code === "KeyA") {
        message = { message: "down", pid: Main.players, uuid: "" };
        flag = 1;
      }
